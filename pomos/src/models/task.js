...
export class Task {
  ...
  constructor({ name, pomodoros, notes = "" }, timer, cloned = false) {
    if (!cloned) this._key = ++Task.lastKey;
  ...
  isPomodoroFinished() {
    if (this.timer == 0) this.increment();
  }
  ...
  increment() {
    if (this.isFinished) {
      console.log(
        `Cannot add more it is full, ${this._current}:${this.pomodoros}`,
      );
      return;
    }
    ...
    console.log(this);
    ++this._current;
    if (this._current >= this.pomodoros) this.isFinished = true;
  }
  ...
  addPomodoros(val) {
    ...
    if (change >= this._current) {
      ...
      if (this.pomodoros == this._current) this.isFinished = true;
      else this.isFinished = false;
    } else console.log("Hey you want to substract more than current", val);
  }
}
