...
export class Timer {
  ...
  constructor(timeInMiliseconds, timeInMilisecondsOriginal = null) {
    ...
    if (!timeInMilisecondsOriginal) this.original = timeInMiliseconds;
    else this.original = timeInMilisecondsOriginal;
  ...
  toggle() {
    if (this.duration == 0) this.refresh();
  ...
  start() {
    ...
    if (this.duration == this.original) this.startedAt = new Date();
  ...
  decrement() {
    ...
    if (result >= 0) this.duration = result;
  }
  ...
  parseTime() {
    ...
    } else AppController.adjustTime();
  }

  pause() {
    this.running = false;
    if (this.duration == 0) console.log("task  finished at:", this.getTime());
    else console.log("Paused at:", this.getTime());
