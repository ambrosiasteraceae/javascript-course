...
function toggleTimer(app) {
  ...
  if (activeTask.isFinished) {
    console.warn(
      "Cannot start a finished task. Increase the number of pomodoros",
    );
    return;
  }
...
export function addRadioClickEvents(app) {
  ...
  for (const taskElement of iter) addRadioClickEvent(app, taskElement);
...
export function attachEventListeners(app) {
  ...
  document.addEventListener("keydown", (e) => {
    if (app.formManager.editMode) return;
