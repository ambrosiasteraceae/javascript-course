...
export function bindFormEvents(fm) {
  ...
  document.addEventListener("keydown", (event) => {
    if (event.key == "Escape") handleTaskEditCancellation(fm);
  });
...
function onOutsideClick(fm) {
  document.body.addEventListener("click", (e) => {
    if (fm.formDiv.classList.contains("hidden")) return;

    if (!fm.isOpen) {
      fm.isOpen = true;
      return;
    }
    if (!fm.form.contains(e.target)) handleTaskEditCancellation(fm);
  });
}
