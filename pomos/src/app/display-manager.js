...
export class DisplayManager {
  ...
  getEditedTask() {
    return [...this.taskElements.values()].filter(
      (ele) => ele.isEdited == true,
    )[0];
  }
  ...
  updateTask(id) {
    ...
    if (taskElement) taskElement.update();
  }
  ...
  getOrder(project) {
    if (this.container.el.children == 0) return project.ordering;
    else {
      project.ordering = [];
  ...
  render(project) {
    ...
    for (const task of iter) this.createTaskElement(task);
    ...
    if (!this.attached) attachDragOverEvent(this, project);
  }
  clear() {
